...
const Connection = () => {
  ...
  const CREATE_PLAYER = gql`
    mutation createPlayer($name: String!, $color: String!) {
      insert_player_one(object: {name: $name, color: $color}) {
        uuid
        name
        color
      }
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (newPlayer.name !== '' && newPlayer.color !== '') {
      createPlayer({ variables: { name: newPlayer.name, color: newPlayer.color } }).then((res) => {
        localStorage.setItem('c', res.data.insert_player_one.uuid);
        navigate('/playground');
      });
    }
  };
